...
const divisibleByFiveTwoToThePower = (input) => {
  // filter out non-numeric and non-divisible by 5 elements
  const filtered = input.map(arr => arr.filter(num => typeof num === 'number' && num % 5 === 0));

  // calculate 2 to the power of each element in filtered arrays
...
let result = findMaleAndFemale(starWarsData);
console.log(result);
...
let findShortest = (data) => {
  ...
  let shortestCharacter = data.reduce((currentShortest, currentCharacter) => {
    // Parse the height strings into integers to compare them
